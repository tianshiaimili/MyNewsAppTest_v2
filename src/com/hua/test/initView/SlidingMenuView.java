...
import com.hua.tes.slidingmenu.SlidingMenu.CanvasTransformer;
...
public class SlidingMenuView {
    ...
    public SlidingMenu initSlidingMenuView(Activity activity, View view,int mScreenWidth) {
        // 菜单打开时的动画
        ...
        slidingMenu.setShadowWidthRes(mScreenWidth / 50);// 设置阴影图片的宽度
        slidingMenu.setShadowDrawable(R.drawable.shadow);// 设置阴影图片
        slidingMenu.setBehindOffsetRes(mScreenWidth / 3);// SlidingMenu划出时主页面显示的剩余宽度
...
//        slidingMenuView.setBehindWidthRes(R.dimen.left_drawer_avatar_size);//设置SlidingMenu菜单的宽度
        slidingMenu.setMenu(LayoutInflater.from(activity).inflate(R.layout.activity_left, null));// 设置menu的布局文件
